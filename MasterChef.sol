...
import "./libs/IPumpkinReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

import "./PumpkinToken.sol";

// MasterChef is the master of Pumpkin. He can make PUMPKIN and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PUMPKIN is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PUMPKIN
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPumpkinPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPumpkinPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. PUMPKINs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that PUMPKINs distribution occurs.
        uint256 accPumpkinPerShare;   // Accumulated PUMPKINs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The PUMPKIN TOKEN!
    PumpkinToken public pumpkin;
    // Dev address.
    address public devAddress;
    // Deposit Fee address
    address public feeAddress;
    // PUMPKIN tokens created per block.
    uint256 public pumpkinPerBlock;
    // Bonus muliplier for early pumpkin makers.
    uint256 public constant BONUS_MULTIPLIER = 1;

    // Initial emission rate: 0.7 PUMPKIN per block.
    uint256 public constant INITIAL_EMISSION_RATE = 700 finney;
    // Minimum emission rate: 0.1 PUMPKIN per block.
    ...
    constructor(
        ...
        pumpkinPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pumpkinReward = multiplier.mul(pumpkinPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pumpkin.mint(devAddress, pumpkinReward.div(10));
        pumpkin.mint(address(this), pumpkinReward);
        pool.accPumpkinPerShare = pool.accPumpkinPerShare.add(pumpkinReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for PUMPKIN allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(pumpkinReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            pumpkinReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPumpkinPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safePumpkinTransfer(msg.sender, pending);
                payReferralCommission(msg.sender, pending);
            }
        }
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(pumpkin)) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPumpkinPerShare).div(1e12);
    ...
    function safePumpkinTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safePumpkinTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(pumpkinPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = pumpkinPerBlock;
        ...
        if (newEmissionRate >= pumpkinPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = pumpkinPerBlock;
        pumpkinPerBlock = newEmissionRate;
